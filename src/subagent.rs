...
//!
//! Pebble builds the [`ChildSessionSpec`] and the application's
//! [`SessionFactory`] turns it into a session. The spec carries the parent's
...
/// [`ChildSessionSpec`], so the limit is a property of the tree rather than of
/// one supervisor.
#[derive(Debug)]
...
/// session needs comes from the spec, which is built from the parent, and a
/// factory that answers with a session it built some other way has its spawn
/// refused rather than supervised.
///
/// ```no_run
/// use std::sync::Arc;
///
...
/// [`SessionFactory`], and the factory turns it into a session with
...
impl fmt::Debug for ChildSessionSpec {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("ChildSessionSpec")
...
impl ChildSessionSpec {
    ...
    /// Builds the child session, ready for its runner to initialize.
    ///
    ...
    /// spec, which carries the parent's own, and where the child sits in its
    /// tree is settled here and cannot be changed afterwards. A factory is the
    /// application's own code inside the parent's trust domain, so those knobs
    /// stay open to it. What it may not do is answer with a *different*
    /// session, which is why a spawn refuses one this spec did not build.
    ///
    /// # Errors
    ///
...
impl SubagentSupervisor {
    ...
    fn spawn_inner(
        ...
        let spec = ChildSessionSpec {
            deps:              Arc::clone(&self.deps),
            parent_session_id: parent_session_id.to_owned(),
            root_session_id:   root_session_id.to_owned(),
            depth:             child_depth,
        };
        let session = (self.deps.factory)(spec).map_err(|error| {
        ...
        // capabilities.
        if !session.was_built_from(&self.deps) {
