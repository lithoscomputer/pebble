...
use crate::subagent::{
    ChildSessionSpec, SessionFactory, SubagentLimits, SubagentResult, SubagentStatus,
    SubagentSupervisor,
};
...
fn recording_factory() -> (SessionFactory, Arc<Mutex<Vec<ChildHandle>>>) {
...
async fn a_childs_events_reach_the_parents_durable_stream() {
    ...
    let mut parent = builder(client)
        ...
        .subagents(Arc::new(ChildSessionSpec::build))
        .build()
