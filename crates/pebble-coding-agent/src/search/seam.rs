//! The seam between the `web_search` tool and a search engine.
//!
//! Pebble owns everything the model sees of a search: the tool, its schema,
//! and the way results are written out. The engine behind it is a
//! [`SearchProvider`] — one the application implements over whatever it has,
//! an API key it holds, a service it runs, a cache it prefers, or one of the
//! providers pebble ships behind the `search-providers` feature.
//!
