...
use pebble_coding_agent::events::{
    CodingAgentEvent, CodingEvent, Emitter, EventOptions, EventPump,
};
...
struct Events {
    emitter:  Emitter,
    pump:     JoinHandle<pebble_coding_agent::Result<()>>,
    received: Receiver<CodingAgentEvent>,
}
...
fn round_for<'a>(events: &'a [CodingAgentEvent], tool_call_id: &str) -> Vec<&'a CodingEvent> {
    events
        .iter()
