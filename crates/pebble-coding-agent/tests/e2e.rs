...
async fn a_prompt_writes_edits_reads_and_runs_a_command_in_its_workspace() {
    ...
    let (mut session, provider) = coding_agent(&workspace, vec![
...
async fn every_tool_call_is_answered_before_the_next_round() {
    ...
    let (mut session, _provider) = coding_agent(&workspace, vec![
        ScriptedCall::response(tool_call_response(
            "write_file",
            "call_write",
            json!({
...
async fn a_steer_sent_while_a_tool_runs_arrives_as_the_next_turn() {
    ...
    let (mut session, provider) = agent_with(
        &workspace,
        vec![
...
async fn a_finished_prompt_reports_what_it_used() {
    ...
    let (mut session, _provider) = coding_agent(&workspace, vec![ScriptedCall::response(
...
) -> (CodingAgent, Arc<ScriptedProvider>) {
    agent_with(workspace, calls, Vec::new()).await
}
...
async fn agent_with(
    ...
    let agent = CodingAgent::builder(client, Arc::new(LocalEnvironment::new(workspace.path())))
        .model("test/model")
        .tools(tools)
        .build()
        .await
